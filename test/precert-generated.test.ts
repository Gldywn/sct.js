...
for (const [alg, log] of Object.entries(fixtures.logs)) {
  describe(`Pre-certificate SCT verification for ${alg}`, () => {
    it('verifies a valid SCT', () => {
      assert.doesNotThrow(() => {
        verifySct(fixtures.scts[alg].valid, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [log]);
      });
    });

    it('fails to verify an SCT with a future timestamp', () => {
      assert.throws(
        () => verifySct(fixtures.scts[alg].future, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [log]),
    ...
    it('fails to verify an SCT with a bad signature', () => {
      assert.throws(
        () => verifySct(fixtures.scts[alg].badsig, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [log]),
    ...
    it('fails to verify an SCT with the wrong log ID', () => {
      assert.throws(
        () => verifySct(fixtures.scts[alg].wrongid, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [log]),
