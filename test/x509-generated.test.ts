...
describe('Generated SCT tests', () => {
  ...
  describe('ecdsa_p256', () => {
    ...
    test('wrongtime', () => {
      ...
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), new Error(VerificationError.InvalidSignature));
    });
  ...
  describe('other failures', () => {
    ...
    test('junk', () => {
      ...
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), new Error(VerificationError.MalformedSct));
    });

    test('wrongid', () => {
      const sct = readTestData('ecdsa_p256-wrongid-sct.bin', 'x509');
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), new Error(VerificationError.UnknownLog));
    });

    test('version', () => {
      const sct = readTestData('ecdsa_p256-version-sct.bin', 'x509');
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), new Error(VerificationError.UnsupportedSctVersion));
    });

    test('future', () => {
      const sct = readTestData('ecdsa_p256-future-sct.bin', 'x509');
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, 1233, logs), new Error(VerificationError.TimestampInFuture));
    });
    ...
    test('short', () => {
      ...
      for (let i = 0; i < sct.length; i++) {
        ...
        assert.throws(() => verifySct(truncatedSct, CERT, ENTRY_TYPE.X509_ENTRY, 1234, logs), new Error(VerificationError.MalformedSct));
      }
    });
  });
});
