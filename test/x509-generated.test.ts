...
describe('X.509 SCT verification with generated data', () => {
  ...
  describe('When using ecdsa_p256', () => {
    ...
    test('wrongtime', () => {
      ...
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), {
  ...
  describe('When handling other failure cases', () => {
    ...
    test('junk', () => {
      ...
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), {
        code: VerificationError.MalformedSct,
      });
    });
    ...
    test('version', () => {
      ...
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, NOW, logs), {
        code: VerificationError.UnsupportedSctVersion,
      });
    });

    test('future', () => {
      const sct = readTestData('ecdsa_p256-future-sct.bin', 'x509');
      assert.throws(() => verifySct(sct, CERT, ENTRY_TYPE.X509_ENTRY, 1233, logs), {
    ...
    test('short', () => {
      ...
      for (let i = 0; i < sct.length; i++) {
        ...
        assert.throws(() => verifySct(truncatedSct, CERT, ENTRY_TYPE.X509_ENTRY, 1234, logs), {
          code: VerificationError.MalformedSct,
        });
      }
    });
