...
describe('Pre-certificate SCT verification with Google test data', () => {
  describe('When verifying SCT 0', () => {
    it('verifies a valid SCT', () => {
      assert.doesNotThrow(() => {
        verifySct(fixtures.sct0, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [fixtures.log0]);
      });
    });
    ...
    it('fails to verify an SCT with a bad signature', () => {
      ...
      assert.throws(
        () => verifySct(badSctBuffer, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [fixtures.log0]),
    ...
    it('fails to verify against the wrong log', () => {
      assert.throws(
        () => verifySct(fixtures.sct0, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [fixtures.log1]),
  ...
  describe('When verifying SCT 1', () => {
    it('verifies a valid SCT', () => {
      assert.doesNotThrow(() => {
        verifySct(fixtures.sct1, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [fixtures.log1]);
      });
    });
    ...
    it('fails to verify an SCT with a bad signature', () => {
      ...
      assert.throws(
        () => verifySct(badSctBuffer, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [fixtures.log1]),
        { code: VerificationError.InvalidSignature },
        'Verification should fail with InvalidSignature',
      );
    });
    ...
    it('fails to verify against the wrong log', () => {
      assert.throws(
        () => verifySct(fixtures.sct1, fixtures.precert, ENTRY_TYPE.PRECERT_ENTRY, Date.now(), [fixtures.log0]),
        { code: VerificationError.UnknownLog },
        'Verification should fail with UnknownLog',
      );
    });
